...
fun Route.eventsRoutes(eventsClient: IEventsClient) {
    route("/api/events") {
        ...
        post {
            ...
            val event = eventsClient.createEvent(request)
        ...
        put("/{id}") {
            ...
            val event = eventsClient.updateEvent(id, request)
            call.respond(event)
        }
        ...
        post("/{id}/publish") {
            ...
            val event = eventsClient.publishEvent(id)
            call.respond(event)
        }
        ...
        post("/{id}/cancel") {
            ...
            val event = eventsClient.cancelEvent(id)
            call.respond(event)
        }
        ...
        post("/{id}/finish") {
            ...
            val event = eventsClient.finishEvent(id)
            call.respond(event)
        }
    }
}
