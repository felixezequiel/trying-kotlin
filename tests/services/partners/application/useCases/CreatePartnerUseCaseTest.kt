...
class CreatePartnerUseCaseTest {
    ...
    fun `deve criar parceiro com sucesso`() = runTest {
        ...
        assertEquals("12345678901234", partner?.document)
        ...
        val exception =
                assertThrows(IllegalStateException::class.java) {
                    kotlinx.coroutines.runBlocking {
                        createPartnerUseCase.execute(userId = 1L, request = request2)
                    }
                }
    ...
    fun `deve falhar quando documento já existe`() = runTest {
        ...
        val exception =
                assertThrows(IllegalStateException::class.java) {
                    kotlinx.coroutines.runBlocking {
                        createPartnerUseCase.execute(userId = 2L, request = request2)
                    }
                }
